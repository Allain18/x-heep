...
|---|---|---|---|
| `STATUS` | `[0]` READY | RW | Reads `1` when the controller is idle and ready for a new transaction. Poll before writing a new transaction. |
...
| `CONTROL` | `[2]` QUAD | RW | `1` = Quad SPI mode, `0` = standard single SPI mode. |
...
3. Write `MD_ADDRESS` — SRAM address of source data (required for writes; set to any valid address for reads).
...
The cache is parameterised by `N_WAYS` and `N_SETS`. The default instantiation is direct-mapped (`N_WAYS=1`) with `N_SETS=4`, giving four independent cache lines of one flash sector (4 KiB) each, 16 KiB total.
...
- Byte-addressable (sub-word) cache access is not yet supported. Byte-granularity transfers bypass the cache.
- Only direct-mapped (`N_WAYS=1`) is implemented. Set-associative operation and replacement policies are planned future work.
