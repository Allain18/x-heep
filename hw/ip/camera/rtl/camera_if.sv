...
#(
  ...
  initial begin
    assert ((DATA_WIDTH == 8) || (DATA_WIDTH == 10))
    else $fatal(1, $sformatf("[CPI] DATA_WIDTH = %0d is not supported", DATA_WIDTH));
  end
...
`endif
  ...
  always_comb begin : proc_format
    ...
    case (s_cfg_format)
      ...
      `BYPASS_LITEND: s_yuv_pix = {r_data_msb[7:0], cam_data_i[7:0]};
      `BYPASS_BIGEND: s_yuv_pix = {cam_data_i[7:0], r_data_msb[7:0]};
      `BYPASS_10BITS: s_byp32_pix = {6'b000000, r_data_msb[9:0], 6'b000000, cam_data_i[9:0]};
  ...
  always_comb begin : proc_sfilter_shift
    case (s_cfg_shift)
      ...
      6: s_data_filter_shift = {5'h0, r_data_filter[16:6]};
      7: s_data_filter_shift = {6'h0, r_data_filter[16:7]};
      8: s_data_filter_shift = {7'h0, r_data_filter[16:8]};
      9: s_data_filter_shift = {8'h0, r_data_filter[16:9]};
  ...
  always_ff @(posedge s_cam_clk_dft or negedge rstn_i) begin : proc_pix
    if (~rstn_i) begin
      ...
      end else r_data_filter_valid <= 1'b0;
    ...
    if (s_framevalid && r_enable) begin
      if (s_cfg_frameslice_en) begin
        if( (r_rowcounter >= s_cfg_frameslice_lly) &&
          ...
          s_pixel_valid = 1'b1;
        else s_pixel_valid = 1'b0;
      end else s_pixel_valid = 1'b1;
    end else s_pixel_valid = 1'b0;
  end
  ...
  always_ff @(posedge s_cam_clk_dft or negedge rstn_i) begin : proc_vsync
    if (~rstn_i) begin
      ...
      if (r_en_sync[1]) r_vsync <= s_cam_vsync;
    end
  end
  ...
  always_ff @(posedge s_cam_clk_dft or negedge rstn_i) begin : proc_sample_lsb
    if (~rstn_i) begin
      r_sample_msb <= 1'b1;
    end else if (~r_enable | ~cam_hsync_i) begin
      r_sample_msb <= 1'b1;
    end else if (cam_hsync_i & r_enable) begin
  ...
  always_ff @(posedge s_cam_clk_dft or negedge rstn_i) begin : proc_data
    if (~rstn_i) begin
      ...
      end else if (~s_sof & ~r_en_sync[1]) begin
        r_rowcounter <= 'h0;
        r_colcounter <= 'h0;
        r_enable     <= r_en_sync[1];  //disable the IP when not SOF
      end else if (cam_hsync_i & r_enable) begin
        if (r_sample_msb) r_data_msb <= cam_data_i;
        if (!r_sample_msb && s_cfg_frameslice_en) begin
          if (r_colcounter == s_cfg_rowlen) begin
            r_colcounter <= 'h0;
            r_rowcounter <= r_rowcounter + 1;
          end else r_colcounter <= r_colcounter + 1;
        end
      end
    end
  end
  ...
  always_ff @(posedge s_cam_clk_dft or negedge rstn_i) begin : proc_framecount
    if (~rstn_i) begin
      ...
      if (s_sof && r_enable) begin
        if (s_cfg_framedrop_en) begin
          if (r_framecounter == s_cfg_framedrop_val) r_framecounter <= 'h0;
          else r_framecounter <= r_framecounter + 1;
        end else r_framecounter <= 'h0;
      end
    end
  end
